...
//        iFragment.setupFragmentComponent(ArmsUtils.obtainAppComponentFromContext(mFragment.getActivity()));
        ...
        if (iFragment instanceof ISimpleFragment) {
            ISimpleFragment iFragmentPresenter = (ISimpleFragment) iFragment;
            IPresenter iPresenter = iFragmentPresenter.obtainPresenter();
            iFragmentPresenter.setPresenter(iPresenter);
        } else if (iFragment instanceof IDaggerFragment) {
            IDaggerFragment iFragmentPresenter = (IDaggerFragment) iFragment;
            //这里提供 AppComponent 对象给 BaseActivity 的子类, 用于 Dagger2 的依赖注入
            iFragmentPresenter.setupFragmentComponent(ArmsUtils.obtainAppComponentFromContext(mFragment.getActivity()));
        }
    }
